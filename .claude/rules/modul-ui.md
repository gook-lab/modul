...
- base 값(테마별 bg·surface·text·accent, 폰트, 반경, 간격, 모션, 이징)의 SSOT 는 `packages/tokens/theme.json` 이다. 바꿀 때는 theme.json 과 배포 CSS 를 같이 고친다 — `pnpm --filter @gook-lab/tokens build` 가 둘을 대조해 어긋나면 실패한다.
