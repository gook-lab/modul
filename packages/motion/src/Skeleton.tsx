...
export function Skeleton({ width = '100%', height = 12, style, className, ...rest }: SkeletonProps) {
  return <div aria-hidden className={`mdl-motion${className ? ` ${className}` : ''}`} style={{ width, height, ...shimmer, ...style }} {...rest} />;
}
...
export function Spinner({ variant = 'ring', size = 24, style, className, ...rest }: SpinnerProps) {
  ...
  return <span role="status" className={`mdl-motion${className ? ` ${className}` : ''}`} style={{ ...s, ...style }} {...rest} />;
}

export function Progress({ value, style, className, ...rest }: { value: number } & ComponentPropsWithoutRef<'div'>) {
  const t = useLabels();
  return (
    // aria-label 은 rest 앞에 둡니다 — 소비자가 준 값이 이깁니다. 이름 없는 progressbar 는 axe aria-progressbar-name 위반입니다.
    <div role="progressbar" className={`mdl-motion${className ? ` ${className}` : ''}`} aria-label={t('progress.label')} aria-valuenow={Math.round(value * 100)} aria-valuemin={0} aria-valuemax={100} style={{ height: 2, background: 'var(--color-neutral-300)', ...style }} {...rest}>
