...
export function useDrag({ snap, rubber = .35, min = Math.min(...snap), max = Math.max(...snap), spring = SPRING }: { snap: number[]; rubber?: number; min?: number; max?: number; spring?: typeof SPRING }) {
  ...
  const bind = {
    onPointerDown: (e: React.PointerEvent) => { (e.currentTarget as HTMLElement).setPointerCapture(e.pointerId); drag.current = { start: e.clientX, from: s.x }; },
    ...
    onPointerUp: () => { if (!drag.current) return; drag.current = null; s.to(snap.reduce((a, b) => Math.abs(b - s.x) < Math.abs(a - s.x) ? b : a)); },
    style: { touchAction: 'pan-y' as const, cursor: drag.current ? 'grabbing' : 'grab' },
  };
  return { x: s.x, bind, dragging: !!drag.current };
}
