  ...
  return (
    <div className={cx('avatar-upload', className)} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
      <label htmlFor={iid} onDragOver={e => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={e => { e.preventDefault(); setDrag(false); take(e.dataTransfer.files[0]); }}
        ...
        {value ? <img src={value} alt="프로필 사진" style={img} /> : <span aria-hidden style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', fontFamily: 'var(--font-heading)', fontWeight: cssVar('--font-heading-weight'), fontSize: size * .32, color: 'var(--color-neutral-700)' }}>{fallback}</span>}
        <span style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: '4px 0', textAlign: 'center', fontSize: 10, letterSpacing: '.06em', textTransform: 'uppercase', background: 'color-mix(in srgb, var(--color-neutral-900) 70%, transparent)', color: 'var(--color-bg)' }}>{drag ? '놓기' : value ? '바꾸기' : '올리기'}</span>
      </label>
...
export function ImageGallery({ value, onChange, max = 5, reorder = true, maxSize = 2e6, accept = 'image/*', onUpload, onError, label, className, ...rest }: ImageGalleryProps) {
  ...
  const move = (from: number, to: number) => { if (from === to) return; const arr = [...value]; const [m] = arr.splice(from, 1); arr.splice(to, 0, m); onChange(arr); };
  return (
    <div role="group" aria-label={typeof label === 'string' ? label : undefined} className={cx('image-gallery', className)} {...rest}>
      {label && <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 12 }}><span style={{ fontSize: 11, letterSpacing: '.08em', textTransform: 'uppercase', color: 'var(--color-neutral-700)' }}>{label}</span><span style={{ fontSize: 11, color: 'var(--color-neutral-700)', fontVariantNumeric: 'tabular-nums' }}>{value.length} / {max}{reorder && ' · 드래그로 순서 변경'} · 첫 장이 대표</span></div>}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(104px, 1fr))', gap: 2 }}>
        {value.map((it, k) => (
          <div key={it.id} draggable={reorder} onDragStart={() => setDragId(it.id)} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); const from = value.findIndex(x => x.id === dragId); if (from >= 0) move(from, k); setDragId(null); }} onDragEnd={() => setDragId(null)}
            ...
            <img src={it.url} alt={it.alt ?? `사진 ${k + 1}`} style={img} />
            ...
            {k === 0 && <span style={{ position: 'absolute', left: 6, top: 6, fontSize: 9, letterSpacing: '.08em', textTransform: 'uppercase', padding: '2px 6px', background: 'var(--color-accent)', color: 'var(--color-bg)' }}>대표</span>}
            <button type="button" aria-label={`사진 ${k + 1} 제거`} onClick={() => onChange(value.filter(x => x.id !== it.id))} style={{ position: 'absolute', right: 4, top: 4, width: 24, height: 24, border: 0, background: 'color-mix(in srgb, var(--color-neutral-900) 70%, transparent)', color: 'var(--color-bg)', cursor: 'pointer', padding: 0 }}>×</button>
          </div>
        ))}
        {value.length < max && (
